...
import { Shield, AlertTriangle, CheckCircle, ExternalLink, Globe } from "lucide-react"
...
const commonTLDs = ["com", "org", "net", "edu", "gov", "mil", "int", "co", "io", "ly", "me"]
...
export default function URLSafetyChecker() {
  ...
  const generateWarnings = (checks: SafetyResult["checks"], urlObj: URL): string[] => {
    const warnings = []

    if (!checks.https) warnings.push("Connection is not encrypted (HTTP instead of HTTPS)")
    if (!checks.validDomain) warnings.push("Domain format appears invalid")
    if (!checks.noSuspiciousKeywords) warnings.push("URL contains suspicious keywords")
    if (!checks.notBlacklisted) warnings.push("Domain is on security blacklists")
    if (!checks.validTLD) warnings.push("Unusual or suspicious top-level domain")
    if (!checks.noPhishing) warnings.push("URL matches known phishing patterns")
    if (isURLShortener(urlObj.hostname)) warnings.push("Shortened URL - destination is hidden")

    return warnings
  }

  const generateRecommendations = (checks: SafetyResult["checks"], urlObj: URL): string[] => {
    const recommendations = []

    if (!checks.https) recommendations.push("Only visit HTTPS websites for secure connections")
    if (isURLShortener(urlObj.hostname)) recommendations.push("Expand shortened URLs before clicking")
    if (!checks.notBlacklisted) recommendations.push("Avoid this website - it's flagged as malicious")

    recommendations.push("Verify the website's legitimacy before entering personal information")
    recommendations.push("Use updated antivirus software and browser security features")
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold mb-4">URL Safety Checker</h1>
          <p className="text-xl text-muted-foreground">Check if a website is safe to visit before clicking</p>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Enter any website URL to analyze its safety</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                ...
                />
                <p className="text-xs text-muted-foreground mt-1">Enter the full URL or just the domain name</p>
              </div>

              <Button onClick={checkURL} disabled={isChecking || !url.trim()} className="w-full">
                {isChecking ? (
                  <>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Detailed security analysis of the URL</CardDescription>
            </CardHeader>
            <CardContent>
              {result ? (
                <div className="space-y-6">
                  <div className="text-center">
                    <div className="flex items-center justify-center gap-2 mb-2">
                      ...
                      <span className={`text-2xl font-bold ${getStatusColor(result.status)}`}>{result.score}/100</span>
                    </div>
                    <Badge variant={result.status === "safe" ? "default" : "destructive"} className="mb-2">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex items-center gap-2">
                        {result.checks.noSuspiciousKeywords ? (
                          <CheckCircle className="h-4 w-4 text-green-600" />
                        ) : (
                          <AlertTriangle className="h-4 w-4 text-red-600" />
                        )}
                        <span>No Suspicious Keywords</span>
                      </div>
                      <div className="flex items-center gap-2">
                        {result.checks.notBlacklisted ? (
                          <CheckCircle className="h-4 w-4 text-green-600" />
                        ) : (
                          <AlertTriangle className="h-4 w-4 text-red-600" />
                        )}
                  ...
                  <div className="flex gap-2">
                    {result.status === "safe" && (
                      <Button onClick={visitURL} className="flex-1">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <ul className="space-y-2 text-sm text-muted-foreground">
                  <li>• URLs with suspicious keywords or misspellings</li>
                  <li>• Non-HTTPS websites requesting personal info</li>
              ...
              <div>
                ...
                <ul className="space-y-2 text-sm text-muted-foreground">
                  ...
                  <li>• Keep browsers and security tools updated</li>
